...
- **Branch / main:** Single full-screen Ink app: `runTui` renders once (no subprocess loop). Dashboard, Settings, Import, Contact, **Jobs** (full Phase-A flow), **Generate** (`runTuiGeneratePdf` + source/flair UI + `pendingJobId` hand-off from Jobs), **Refine** (Q&A → diff → save; `refined.md` / `refined.json` sync prompt when md is newer), and **Profile** (summary, experience bullets, **skills** list: add/delete/reorder `[`/`]`, inline edit; save to source or refined) are functional in-tree. No `DelegateScreen`, `exitBag`, or `cliArgs.ts`.
