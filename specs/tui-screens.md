...
**Cover letter *(specified — implementation when scheduled)*:** **Jobs** is the **primary** place to **edit** the per-job cover letter draft (multiline editor; single persisted buffer per job). The screen **SHOULD** expose **Refine cover letter (light)** and **AI sniff pass — cover letter** as **separate** actions (diff / accept before save; **Abort** during model work), consistent with the Refine hub’s sniff semantics. **Generate** exposes optional **also export cover letter PDF** when the draft qualifies. Normative rules: [`cover-letter-pdf.md`](./cover-letter-pdf.md).
