...
**Components:** Without refined: **`TextViewport`** + **`ScrollView`** for **read-only** wrapped markdown (rows use dim/bold/italic hints for headings, inline `**` / `` ` `` / list markers, etc., not a rendered markdown preview). With refined: **`FreeCursorMultilineInput`** (**plain** literal display markdown — one cell per character, no hint elision — so mouse/caret/selection align; debounced `onChange`, **`mdExternalRevision`** for external replaces only), **`stripHtmlCommentsFromProfileMarkdown`** (no `<!-- … -->` in text; **comment-only lines removed** from the buffer; **CRLF / lone CR normalized to LF** so logical lines match the TUI buffer). The resume editor’s **`height`** (logical line slots) is derived from **`measureElement`** on the flex-allocated host minus the section strip block and **`TextViewport`** frame rows, so the row count matches the space left after Status / Health / suggested-next and other siblings (not `termRows` minus a single global constant). Save uses **`parseDisplayMarkdownStringToProfile`** then **`profileMarkdownContent`** to restore ids and **`src`** tags (diff vs editor text shows metadata). Section strip uses **`resumeExperiencePositionIdForEditorView`** under Experience. Inline polish **diff** + **`DiffView`**, **`StatusBadge`**, health/validation, suggested next. Pipeline strip on shell **StatusBar**. **Mouse wheel** scrolls the resume pane when that pane is scroll-active (read-only view, or editor with body focus). Navigation: **Esc** leaves editor focus for **`:`** palette, **`1–n`**, letter keys; **Tab** refocuses the editor; **`o`** / **Ctrl+O** outline; **Ctrl+S** save; **Ctrl+P** / **Ctrl+E** when API + body focused (see document shell §1, §8). Manual structured profile edit still under **Refine** / **Profile editor**.
...
**Cover letter PDF *(specified — implementation when scheduled)*:** When a **saved job** is selected and a **non-empty** per-job cover letter draft exists, **Generate** **SHOULD** offer **“Also export cover letter PDF”** (or equivalent). Export **MUST NOT** run for an empty draft. Authoring and **light refine** / **AI sniff pass** for the letter live primarily on **Jobs** (see **JobsScreen** below); normative product rules: [`cover-letter-pdf.md`](./cover-letter-pdf.md).
...
**Cover letter (implemented):** **`l`** in job nav mode opens [`CoverLetterEditor`](../src/tui/components/CoverLetterEditor.tsx), a **full-screen** word-wrapped markdown editor (`FreeCursorMultilineInput` with `wordWrap`). The editor opens in **nav mode** — **Tab** enters text editing, **Esc** returns to nav mode. Nav-mode keybinds: **`r`** light refine, **`n`** AI sniff pass, **`s`** save, **Esc** close. Both AI assists show a **DiffView** review step (accept/reject) before save. If the draft is empty on open, the editor **auto-generates** from the user’s profile + JD. Parent `ResumeEditor` input handlers are deactivated while the cover letter editor is open (`!coverLetterOpen` guard). **Generate** exposes optional **also export cover letter PDF** when the draft qualifies. Normative rules: [`cover-letter-pdf.md`](./cover-letter-pdf.md).
