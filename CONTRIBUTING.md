...
```bash
pnpm install
pnpm run ci               # tests + check:tui-imports + build (recommended before push)
pnpm check                # Biome on src/ + scripts/ (see note below)
```

`package.json` **`ci`** script runs `pnpm test`, **`pnpm check:tui-imports`** (TUI must not import `commands/` or Inquirer), and **`pnpm build`**. Use `pnpm run ci` (not the built-in `pnpm ci`, which is different).
