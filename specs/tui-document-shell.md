...
- **Dashboard as hub:** workflow overview with pipeline status and navigation — no editor on the dashboard. See [`dashboard-editor-redesign.md`](./dashboard-editor-redesign.md).
- **Shared `ResumeEditor` component:** continuous markdown **viewport** rendered from **in-memory `Profile`** via `profileMarkdownContent` ([`src/profile/markdown.ts`](../src/profile/markdown.ts)). Used by both the **Editor** screen (general resume) and **Jobs** screen (job-specific). When **`hasRefined`**, the editor is an **editable** markdown buffer: **`parseMarkdownStringToProfile`** round-trips edits before save; the **caret’s current `##` section** selects **polish** scope and **section consultant** handoff (**Ctrl+P** / **Ctrl+E** while focused). Under **`## Experience`**, the nearest preceding **`<!-- pos-id:… -->`** marker scopes to that role only. **Ctrl+S** saves to **`persistenceTarget`**; **Ctrl+O** opens the outline. The markdown field uses **external content revision** so debounced state does not overwrite in-flight typing; caret does not blink.
- **Refine is contextual:** polish, consultant, Q&A, sniff, direct edit, and history are actions within the editor (keybinds + palette), not a separate screen. Onboarding flow chains Q&A → polish → consultant → sniff on first refinement.
...
- **Palette (`:`)** and **Ctrl-?** help (fallbacks: `?`, `h`, palette **Help**) for global actions, plus editor-contextual commands (`:sniff`, `:edit`, `:qa`, `:history`, `:sections`).
...
- **Collapsible JD pane:** In job mode, `Ctrl+J` toggles job description visibility (hidden → peek → full). Side-by-side at wide terminals.
...
|-----------|----------------|
| Dashboard as hub | Pipeline status + navigation; editor is a separate screen. |
| Document first | One scrollable viewport in the editor; outline/jump for navigation. |
...
| Refine is contextual | Polish, consultant, Q&A, sniff, direct edit are editor actions, not a destination. |
| Progressive disclosure | Overlays + palette for Import, Settings, Contact, Generate. Editor keybinds + palette for refine tools. |
...
> **Redesign:** See [`dashboard-editor-redesign.md`](./dashboard-editor-redesign.md) for the normative screen structure.

| Surface | Behavior |
|---------|----------|
| **Dashboard** | Workflow hub — pipeline status, profile identity, `SelectList` of actions. No editor. Default screen; Esc from other screens returns here. |
| **Editor** | General resume editing. Wraps shared `ResumeEditor` with `mode: ‘general’`. Refine tools (polish, consultant, Q&A, sniff, direct edit, history) as contextual keybinds/palette commands. When no refined data: read-only source + onboarding CTA. |
| **Jobs** | Job list → select job → `ResumeEditor` with `mode: ‘job’` + collapsible JD pane (`Ctrl+J`). Job actions as nav-mode keybinds (`p` prepare, `f` feedback, `g` generate, `l` cover letter). |
| **Import / Settings / Contact / Generate** | **Full-viewport overlays** (stacked on **Dashboard**, **Editor**, or **Jobs** as underlay); TopBar shows **effective** screen; **Esc** pops one overlay. **`SET_SCREEN`** clears the overlay stack. |
| **Profile editor** | Nested flow from editor palette `:sections`; same save rules as session target. |

**Empty state:** No `source.json` → Dashboard shows **Import** CTA as primary action.

**`ResumeEditor` scroll and editing:** The shared editor uses **logical-line** scroll inside `FreeCursorMultilineInput`. The editor reserves the **rightmost column** inside the frame for a **scroll thumb** (track + proportional thumb); editable text uses one fewer column than the framed inner width. The block caret uses the same **inverse** (or bold in `NO_COLOR`) styling on **spaces** as on visible characters. **Mouse wheel** scrolls when the editor body is active (SGR mouse mode on stdout). Left **click** moves the **caret**; **click-and-drag** extends a **text selection**. The editor shows **literal display markdown** (after [`stripHtmlCommentsFromProfileMarkdown`](../src/profile/markdown.ts)): **no** hint rendering that elides `**`, `` ` ``, etc., so **one terminal cell per buffer character**. On save, [`profileMarkdownContent`](../src/profile/markdown.ts) reinserts `<!-- ... -->` and `src` annotations. Mouse mode includes **button-motion** (`CSI ? 1002 h`). **Non-wheel** SGR mouse input is **consumed** by the focused handler and **must never** appear as literal characters. The markdown viewport applies **lightweight terminal styling** (dim `#` markers, bold heading titles) without full rendering.

**Generate precedence:** If TopBar has a job, Generate wizard **defaults** to it; **`pendingJobId`** from Jobs handoff still supported.
...
1. **`ResumeSectionId`** (document shell MVP) — Stable string ids for top-level resume blocks emitted as `##` headings from `profileMarkdownContent`: **`summary`**, **`experience`**, **`skills`**, optional **`education`**. Pure index: [`src/tui/resumeSectionIndex.ts`](../src/tui/resumeSectionIndex.ts) (`buildResumeSectionIndex`, `findDisplayRowForSection`, `buildSectionScrollRowMap`, `resumeSectionIdAtMarkdownOffset`, `resumeExperiencePositionIdAtMarkdownOffset` for canonical markdown, **`resumeExperiencePositionIdForEditorView`** when HTML comments are hidden, `experiencePositionShortLabel`). Labels align with Refine polish / section consultant strings ([`refineSectionMenu.ts`](../src/tui/refineSectionMenu.ts)).
2. **Experience role scope** — On disk, each position block is preceded by **`<!-- pos-id:{Position.id} -->`**. The **refined editor** uses [`stripHtmlCommentsFromProfileMarkdown`](../src/profile/markdown.ts): inline comments are removed from lines that still have text; **physical lines that are only metadata** (e.g. a lone `<!-- pos-id:… -->` row) are **dropped** so the buffer has no empty “hidden” rows and caret / scroll / mouse stay aligned with visible lines. While the caret is under **`## Experience`**, **`resumeExperiencePositionIdForEditorView`** maps **`### Title at Company`** blocks (and heading text match to **`profile.positions`**) to **`positionId`**. Canonical markdown still uses **`pos-id`** comments; read-only / disk paths use **`resumeExperiencePositionIdAtMarkdownOffset`**. Scoped polish/consultant use the resolved id; if none (above first role block), polish/consultant apply to **all** experience — strip shows **`Experience (pick a role block)`**.
3. **ResumeEditor — read-only vs editor** — The shared `ResumeEditor` component (see [`dashboard-editor-redesign.md` §4](./dashboard-editor-redesign.md#4-shared-resumeeditor-component)) is used by both the **Editor** screen and **Jobs** screen. When **`hasRefined` is false**, the pane is a **wrapped read-only** `TextViewport` + `ScrollView` (same outline / **Enter** on heading behavior as before). When **`hasRefined`** and refined state has loaded, the pane is **`FreeCursorMultilineInput`** ([`src/tui/components/shared/FreeCursorMultilineInput.tsx`](../src/tui/components/shared/FreeCursorMultilineInput.tsx)): logical-line editing, **Backspace** and the usual **Delete** key (DEL / `0x7f`, including Kitty CSI-u **127**) erase **before** the caret — Ink maps that byte to `key.delete`; we correct via raw stdin + [`stdinBackspaceAsDelete127.ts`](../src/tui/stdinBackspaceAsDelete127.ts). True **forward delete** (`ESC [ 3 ~`, Fn+Delete on many Mac terminals) still erases **after** the caret. **Esc** clears resume body focus (`resumeBodyFocused`) so **`inTextInput`** is false and global **1–n**, **:** palette, and letter jumps work; **Tab** refocuses the editor. Refocus also resets when **`persistenceTarget`** or **`profileDir`** changes. **Ctrl+S** parse + save via **`parseDisplayMarkdownStringToProfile`** (display markdown) + **`saveRefinedForPersistenceTarget`** (persisted body from **`profileMarkdownContent`**, which rewrites **`<!-- ... -->`** and **`src`** metadata), **Ctrl+P** / **Ctrl+E** (API required for polish) scoped by §8.1–8.2, **Ctrl+O** outline jump (plain **`o`** inserts while `inTextInput`). Inline **polish diff** uses **`DiffView`** + accept/discard. Parent passes **`externalContentRevision`** (increment only on load / save normalize / polish accept / reset) so debounced `onChange` → `mdDraft` does **not** overwrite the buffer; caret is **not** blinked (reduces full redraws that clear terminal selection). **Job mode:** collapsible JD pane via `Ctrl+J` (hidden → peek → full); job-specific keybinds (`p`/`f`/`g`/`l`) in nav mode.
