...
## Phase D — Document shell (target TUI)

**Normative spec:** [`tui-document-shell.md`](./tui-document-shell.md). Ship when **all** of the following are true (incremental PRs OK):

- [x] **DocumentShell** is the default `Resume` experience: **Profile→markdown** viewport (read-only without refined; **`FreeCursorMultilineInput`** + section strip + **`mdExternalRevision`** + **Ctrl+S/P/E/O** when refined), **active document session** (`persistenceTarget`), **no cross-write** tests (`saveRefinedForPersistenceTarget`, `loadRefinedTuiState`, contact persistence). **In-repo:** `Layout` → `DocumentShell`, **`TextViewport`** / **`FreeCursorMultilineInput`**, **`o`** / **Ctrl+O** outline + **Enter** on heading (read-only) → scoped actions → **`refineResumeIntent`** (optional **`positionId`** for experience) + Refine auto-flow, **`resumeScrollMemory`** for read-only path. **Not shipped:** optional **side-by-side outline column** when width allows (shell §5) — jump list only today.
- [x] **TopBar** = screen + **Job:** only; **StatusBar** = single row (notifications left, pipeline/health right); legacy **Header pipeline strip** + **two-line Footer** removed or reduced to compatibility shim. **In-repo:** `DocumentShell` chrome + dim baseline/context lines; **`Header.tsx`** unused in the app tree.
- [x] **`:`** palette is **P0** with Refine hub + dashboard-equivalent entries; **Ctrl-?** help + **`?`/`h`/Help** fallbacks per [`tui-README.md`](./tui-README.md). **In-repo:** `CommandPalette` + `App` wiring; palette **Close overlays** when stack non-empty.
- [x] **Import / Settings / Contact / Generate** run as **overlays** with documented Esc stack. **In-repo:** `overlayStack` + **`PUSH_OVERLAY` / `POP_OVERLAY` / `CLEAR_OVERLAYS`**; push from **dashboard** or **Jobs** underlay only (Refine/Profile still full **`SET_SCREEN`** for those targets). **Esc** / palette documented in [`tui-architecture.md`](./tui-architecture.md) + [`tui-document-shell.md`](./tui-document-shell.md).
- [x] **Section-scoped professional review** (Refine hub) + **Resume handoff:** **Section consultant** from Refine; from **Resume** (heading menu or **refined editor** **Ctrl+E**), **Polish** / **consultant** with optional **per-role experience** scope (`pos-id`, **`refineResumeIntent.positionId`**) — see [`tui-document-shell.md`](./tui-document-shell.md) §8, §12.
- [x] Specs in [`tui-document-shell.md`](./tui-document-shell.md) §13 + related docs track tests; **`pnpm vibecheck`** green. **Coverage:** persistence + **`resumeScrollMemory.test.ts`** + **NO_COLOR** (`noColorEnv.test.ts`, `pipelineStrip.test.ts`) + **`consultant.section.test.ts`** + **`refineSectionMenu.test.ts`** + **`resumeSectionIndex.test.ts`** + **`freeCursorLayout.test.ts`**. **Optional later:** broader **NO_COLOR** matrix (badges, modals) and **section-index** edge cases.

**Curate sidebar row:** **Superseded** by job context on document — remove from “must ship” list when Phase D completes; per-job history remains optional per [`refinement-history.md`](./refinement-history.md).
...
- **Curate screen (sidebar)** — **Superseded** for **target** UI by [`tui-document-shell.md`](./tui-document-shell.md) (job target on **TopBar** + same document UX). Legacy spec: [CurateScreen](./tui-screens.md#curatescreen-planned). Phase D checklist above.
...
- [x] **Seven** top-level screens reachable via **command palette (`:`)** and **`1–7`** keys (legacy order); **manual section edit** opens from **Refine**. Pressing a key navigates without crash.
