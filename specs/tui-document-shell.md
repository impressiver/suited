...
**Resume scroll:** **Read-only** wrapped viewport restores **scroll offset** after overlay pop (remount); in-memory **per `profileDir`** only ([`resumeScrollMemory`](../src/tui/resumeScrollMemory.ts)). The **refined markdown editor** uses **logical-line** scroll inside `FreeCursorMultilineInput` (not the same wrapped offset map). The editor reserves the **rightmost column** inside the frame for a **scroll thumb** (track + proportional thumb); editable text uses one fewer column than the framed inner width. The block caret uses the same **inverse** (or bold in `NO_COLOR`) styling on **spaces** as on visible characters. **Mouse wheel** scrolls the resume viewport when the read-only document or the refined editor body is active (SGR mouse mode on stdout while that pane owns scroll; terminal must support xterm-style wheel reporting). **Refined editor:** left **click** moves the **caret**; **click-and-drag** extends a **text selection** (highlighted in the viewport). The editor shows **literal display markdown** (same characters as the in-memory buffer after [`stripHtmlCommentsFromProfileMarkdown`](../src/profile/markdown.ts)): **no** hint rendering that elides `**`, `` ` ``, list markers, etc., so **one terminal cell per buffer character** and pointer/selection offsets stay aligned. Typing, **Enter**, **Backspace**, and **Delete** replace or remove the selection as expected. **Pointer → buffer** mapping uses Yoga layout of the **first visible editor line** `Box` (same row Ink draws as viewport line 0; see [`editorViewportLayout.ts`](../src/tui/utils/editorViewportLayout.ts)), not a coarse wrapper rect, plus clamped scroll read at **event** time so drags stay aligned after wheel scroll. Startup disables SGR **pixel** mouse (`CSI ? 1016 l`) so coordinates stay **character cells**. On save, [`profileMarkdownContent`](../src/profile/markdown.ts) reinserts `<!-- ... -->` and `src` annotations — a normal **diff** against the editor text surfaces metadata as added lines/segments. Mouse mode includes **button-motion** (`CSI ? 1002 h`) so drag events report while the button is held. **Non-wheel** SGR mouse input is **consumed** by the focused handler (caret/selection in the editor; read-only pane ignores pointer) and **must never** appear as literal characters in the buffer (Ink’s `useInput` often delivers CSI without a leading ESC). The markdown viewport applies **lightweight terminal styling** (dim `#` markers, bold heading titles, `**` / `` ` `` / list markers as hints) without rendering full markdown preview.
