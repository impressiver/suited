...
- **`src/tui/screens/RefineScreen.tsx`** — `loadSource` → already-refined menu (Q&A, **polish**, **direct edit**) + md/json sync → Q&A → `applyRefinements` → `DiffView` + **`SelectList`** → `saveRefined` / **keep-session** saves for polish & direct edit; `profileDir` from `App`.
